...
def test_openapi_json_documents_core_endpoints(client):
    ...
    assert "/api/tasks/{task_id}/start" in paths
    assert "/api/tasks/{task_id}/complete" in paths
    assert "/api/errors/search" in paths
    assert "/api/errors/similar" in paths
    assert "/api/ai/daily-briefing" in paths
    assert "/api/machines/{machine_id}/assistant" in paths
    assert "/api/inventory/forecast" in paths
...
def test_openapi_examples_are_present(client):
    ...
    task_example = spec["paths"]["/api/tasks"]["post"]["requestBody"]["content"][
        "application/json"
    ]["example"]
    briefing_example = spec["paths"]["/api/ai/daily-briefing"]["get"]["responses"][
        "200"
